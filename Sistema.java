...
public class Sistema {
	...
	public class GM {
		...
		public GM(Memory _mem, int _tamPag) {
			this.mem = _mem;
			this.tamMem = _mem.pos.length;
			this.tamPag = _tamPag;
			this.nroFrames = tamMem / tamPag;
			this.tabelaPag = new int[nroFrames];
		...
		public int[] aloca(int nroPalavras) {
			int nroPaginas = nroPalavras / tamPag;
		...
		public void descaloca(int[] tabelaPaginas) {
			for (int i = 0; i < tabelaPaginas.length; i++) {
				tabelaPag[tabelaPaginas[i]] = -1;
			}
		}
	}
	...
	public class CPU {
		...
		public CPU(GM gm, Memory _mem, boolean _debug) { // ref a MEMORIA passada na criacao da CPU
	...
	public class HW {
		...
		public HW(int tamMem,int tamPag) {
			...
			this.gm = new GM(mem, tamPag);
			cpu = new CPU(gm, mem, true); // true liga debug
		}
	}
	// -------------------------------------------------------------------------------------------------------
	...
	public class Utilities {
		...
		private void loadProgram(Word[] p) {
			...
			int[] tabelaPaginas = gm.aloca(p.length); // aloca espaço para o programa
			if(tabelaPaginas.length == 0) return;

			int countP = 0;
			for(int pagina = 0; pagina < tabelaPaginas.length; pagina++){
				int inicioDaPaginaAtual = tabelaPaginas[pagina] * gm.tamPag;
				int fimDaPaginaAtual = (pagina + 1) * gm.tamPag;
				for (int i = inicioDaPaginaAtual; i < fimDaPaginaAtual; i++) {
					m[i] = p[countP];
					countP++;
				}
			}
	...
	public Sistema(int tamMem, int tamPag) {
		hw = new HW(tamMem, tamPag);           // memoria do HW tem tamMem palavras
		so = new SO(hw);
